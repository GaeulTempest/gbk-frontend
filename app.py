...
with tab_game:
    ...
    # ------------------------------------------------------------------
    # Camera section (persistent container)
    cam_container = st.container()

    ready_info = ("Waiting opponent..." if not players.get("A",{}).get("ready") or
                                         not players.get("B",{}).get("ready")
                  else "Both players ready!")
