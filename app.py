...
def _hash(pl): return json.dumps(pl, sort_keys=True)

def set_players(new_pl):
    st.session_state.players = new_pl
    st.session_state._players_hash = _hash(new_pl)
    ...
    with cB:
        ...
        if st.button("Join Room") and room:
            res = api_post(f"/join/{urllib.parse.quote(room.strip())}",
                           player_name=name)
            if res: st.session_state.update(res, game_id=room)
            else:   st.error(st.session_state.last_error)
...
with tab_game:
                    ...
                    try:
                        async with websockets.connect(WS_URI, ping_interval=20) as ws:
                            while True:
                                ...
                                if _hash(data["players"]) != st.session_state._players_hash:
    ...
    # Ready button (no experimental_rerun inside)
    if not my_ready:
    ...
    if my_ready and not both_ready and time.time()- (st.session_state.poll_ts or 0) > 3:
        ...
        if snap and _hash(snap["players"]) != st.session_state._players_hash:
