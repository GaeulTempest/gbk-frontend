...
def post(path, **data):
    try:
        ...
        st.session_state.err = e.response.text if hasattr(e,'response') and e.response else str(e)
        return None
...
def h(pl): return json.dumps(pl, sort_keys=True)
def set_players(pl):
    st.session_state.players = pl
    st.session_state._hash   = h(pl)
...
with tab_game:
    ...
    # --- WebSocket listener (re-run when state changes) ---
                    ...
                    try:
                        async with websockets.connect(WS_URI,ping_interval=20) as ws:
                            while True:
                                ...
                                if h(data["players"]) != st.session_state._hash:
    ...
    if not me_ready:
        if st.button("I'm Ready", key=f"ready_{st.session_state.player_id}"):
            ...
            if snap:
                ...
                st.experimental_rerun()      # <- rerun di akhir handler
            else:
                st.error(st.session_state.err)

    # ----- Polling fallback setiap 3 dtk -----
    if not both_ready and time.time() - (st.session_state.poll_ts or 0) > 3:
        st.session_state.poll_ts = time.time()
        snap = get_state(gid)
        if snap and h(snap["players"])!=st.session_state._hash:
